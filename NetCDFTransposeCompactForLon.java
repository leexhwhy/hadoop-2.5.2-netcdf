...
public class NetCDFTransposeCompactForLon {
    private static final Log LOG = LogFactory.getLog(NetCDFTransposeCompactForLon.class);
    ...
    public static class VariableMapper
        ...
        public void map(Text key, NetCDFArrayWritable value, Context context )
            ...
            int chunkSize = (timeSize*latSize*4);
            int numChunksPerKey = (int)(blockSize / chunkSize);

            for( int i = 0; i < lonSize; i++ ){
                FloatWritable[] result = new FloatWritable[2+latSize];
                ...
                System.out.println( "[SAMAN][NetCDFTransposeCompactForLon][Map] key="+(i/numChunksPerKey) );
                java.lang.System.arraycopy(records, i*latSize, result, 2, latSize);
    ...
    public static class MergeChunkReducer
        ...
        public void reduce(Text key, Iterable<NetCDFArrayWritable> values,
            ...
            System.out.println( "[SAMAN][NetCDFTransposeForLon][Reducer] Reducer Beginning!" );
            ...
            int chunkSize = (timeDim*latDim*4);
            int numChunksPerKey = (int)(blockSize / chunkSize);

            System.out.println("[SAMAN][NetCDFTransposeForLon][Reducer] " +
                    "timeDim=" + timeDim + ",latDim=" + latDim + ",lonDim=" + lonDim);

            System.out.println("[SAMAN][NetCDFTranposeCompactForLon][reduce] blockSize=" + blockSize + ",chunkSize=" + chunkSize + ",numChunksPerKey=" + numChunksPerKey);
            ...
            while( itr.hasNext() ){
                NetCDFArrayWritable array = (NetCDFArrayWritable)itr.next();
                System.out.println( "[SAMAN][NetCDFTransposeCompactForLon][reduce] lonIndex="+array.get()[0]+", timeIndex="+array.get()[1] );
            }
            ...
            */
            ...
            System.out.println( "[SAMAN][NetCDFTransposeCompactForLon][Reducer] Reducer Ending!" );
