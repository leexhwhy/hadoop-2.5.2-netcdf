...
public abstract class FileInputFormat<K, V> extends InputFormat<K, V> {
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    ...
    while( itr.hasNext() ){
      ...
      SplitLocationInfo splitLocationInfo[] =  split.getLocationInfo();
      String result = new String();
      for( int i = 0; i < splitLocationInfo.length; i++ ){
        SplitLocationInfo sli = splitLocationInfo[i];
  ...
  protected int getBlockIndex(BlockLocation[] blkLocations, 
                              long offset) {
    //LOG.info("[SAMAN] FileInputFormat.getBlockIndex, blkLocations size: " + blkLocations.length);
    //System.out.println("[SAMAN] FileInputFormat.getBlockIndex, blkLocations size: " + blkLocations.length);
/*    for( int i = 0; i < blkLocations.length; i++ ){

        LOG.info("[SAMAN] FileInputFormat.getBlockIndex, blkLocations["+i+"].length = " + blkLocations[i].getLength() +
            ", blkLocations["+i+"].offset = " + blkLocations[i].getOffset());
        System.out.println("[SAMAN] FileInputFormat.getBlockIndex, blkLocations[" + i + "].length = " + blkLocations[i].getLength() +
                ", blkLocations[" + i + "].offset = " + blkLocations[i].getOffset());
    } */
    for (int i = 0 ; i < blkLocations.length; i++) {
