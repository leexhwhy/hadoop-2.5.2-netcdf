 ...
 */
public class NetCDFOutputFormatCompactForTime<Text, List> extends FileOutputFormat<Text, List> {
    ...
    private static final Log LOG = LogFactory.getLog(NetCDFOutputFormatCompactForTime.class);


    public NetCDFOutputFormatCompactForTime(){
        super();
    }

    protected static class NetCDFRecordWriterCompactForTime<Text, List> extends RecordWriter<Text, List> {
        ...
        public NetCDFRecordWriterCompactForTime( FileSystem _fs, TaskAttemptContext _job ) {
        ...
        public synchronized void write(Text key, List value)
            ...
            System.out.println( "[SAMAN][NetCDFRecordWriterCompactForTime][write] Beginning!" );
            ...
            String currentCumulativeTime = keySplitted[0];
            String timeDimSize = keySplitted[1];
            String latDimSize = keySplitted[2];
            String lonDimSize = keySplitted[3];
            System.out.println( "Cumulative Time is: "+keySplitted[0]+",timeDim: "+keySplitted[1]
                    +",latDim: "+keySplitted[2]+",lonDim: "+keySplitted[3] );

            int blockSize = 512*1024*1024;
            int chunkSize = Integer.valueOf(latDimSize)*Integer.valueOf(lonDimSize)*4;
            ...
            int timeIndexesSize = ((Integer.valueOf(currentCumulativeTime)+1)*numChunksPerKey <= Integer.valueOf(timeDimSize))
                    ? ( numChunksPerKey )
                    : ( Integer.valueOf(timeDimSize)-(Integer.valueOf(currentCumulativeTime))*numChunksPerKey );

            System.out.println( "[SAMAN][NetCDFOutputFormatCompactForTime][Write] timeIndexesSize="+timeIndexesSize );
            ...
            String outputFileName = "/data/saman/time-" + currentCumulativeTime + ".nc";
            ...
            try {
                ...
                Dimension timeDim = outputFile.addDimension(null, vtime.getDimensionsString(), timeIndexesSize);
                ...
                Array dataTime = Array.factory(DataType.DOUBLE, new int[]{timeIndexesSize});
                ...
                for( int i = 0; i < timeIndexesSize; i++ ){
                    ...
                    dataTime.setDouble(i, Double.valueOf(timeArray.get(Integer.valueOf(currentCumulativeTime)*numChunksPerKey+i)));
                }
                ...
                Array dataTimeBnds = Array.factory(DataType.DOUBLE, new int[]{timeIndexesSize, 2});
                //long first122 = System.nanoTime();
                shape = dataTimeBnds.getShape();
                Index2D idx = new Index2D(new int[]{timeIndexesSize, 2});
                //idx.set(0, 0);
                ...
                for (int i = 0; i < timeIndexesSize; i++) {
                    for (int j = 0; j < shape[1]; j++) {
                        idx.set(i, j);
                        dataTimeBnds.setDouble(idx, timeBndsArray.get(Integer.valueOf(currentCumulativeTime)*numChunksPerKey+i, j));
                    }
                }
                ...
                Index3D idx3 = new Index3D(new int[]{timeIndexesSize, (int) (vlat.getSize()), (int) (vlon.getSize())});
                Array dataRsut = Array.factory(DataType.FLOAT, new int[]{timeIndexesSize, (int) (vlat.getSize()), (int) (vlon.getSize())});
                int globalIndex = 0;

                for( int i = 0; i < timeIndexesSize; i++ ) {
                    for (int j = 0; j < vlat.getSize(); j++) {
                        ...
                        for (int k = 0; k < vlon.getSize(); k++) {
                            try {
                                ...
                                dataRsut.setFloat(idx3, records[1+k].get());
                ...
                _fs.copyFromLocalFile(new Path(outputFileName), new Path(_output_path + "/rsuttime" + currentCumulativeTime));
    ...
    public RecordWriter<Text, List> getRecordWriter(TaskAttemptContext job) throws
        ...
        return new NetCDFRecordWriterCompactForTime<Text, List>( _fs, job );
