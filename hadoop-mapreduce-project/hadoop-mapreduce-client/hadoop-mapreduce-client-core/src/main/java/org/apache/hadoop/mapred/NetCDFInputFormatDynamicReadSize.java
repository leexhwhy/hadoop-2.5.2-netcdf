 ...
 */
public class NetCDFInputFormatDynamicReadSize extends FileInputFormat<Text, NetCDFArrayWritable> {

    private static final Log LOG
            = LogFactory.getLog(NetCDFInputFormatDynamicReadSize.class.getName());
    ...
    public InputSplit[] getSplits(JobConf job, int numSplits)
        ...
        LOG.info("[SAMAN][NetCDFInputFormatDynamicReadSize][getSplits] hive query is: " + job.get(HIVE_QUERY, "Kossher"));
        System.out.println("[SAMAN][NetCDFInputFormatDynamicReadSize][getSplits] hive query is: " + job.get(HIVE_QUERY, "Kossher"));
        ...
        for (FileStatus file: files) {
            ...
            LOG.info("[SAMAN][NetCDFInputFormatDynamicReadSize][getSplits] File name is : " + path.getName());
            System.out.println("[SAMAN][NetCDFInputFormatDynamicReadSize][getSplits] File name is : " + path.getName());
            ...
            if ((length != 0) && isSplitable(fs, path)) {
                ...
                long   thisChunk       = 0;
                long   blockNo         = 1;
                LOG.info( "[SAMAN] NetCDFInputFormatDynamicReadSize.getSplits => recStart = " + recStart + ", chunkStarts = " + chunkStarts +
                        ", smallSize = " + smallSize + ", recSize = " + recSize + ", bytesRemaining = " + bytesRemaining +
                        ", thisStart = " + thisStart);
                System.out.println( "[SAMAN] NetCDFInputFormatDynamicReadSize.getSplits => recStart = " + recStart + ", chunkStarts = " + chunkStarts +
                        ", smallSize = " + smallSize + ", recSize = " + recSize + ", bytesRemaining = " + bytesRemaining +
                        ", thisStart = " + thisStart);
                while ( bytesRemaining > 0) {
                    ...
                    endChunk            = chunkIndex;
                    blockNo++;
                    LOG.info( "[SAMAN] NetCDFInputFormatDynamicReadSize.getSplits => splitSize="+splitSize+", thisStart="+thisStart+
                            ", endChunk="+endChunk+", blockNo="+blockNo);
                    System.out.println( "[SAMAN] NetCDFInputFormatDynamicReadSize.getSplits => splitSize="+splitSize+", thisStart="+thisStart+
                            ", endChunk="+endChunk+", blockNo="+blockNo);
                    ...
                    thisChunk = endChunk;
                    LOG.info( "[SAMAN] NetCDFInputFormatDynamicReadSize.getSplits => bytesRemaining="+bytesRemaining+", thisChunk="+thisChunk );
                    System.out.println( "[SAMAN] NetCDFInputFormatDynamicReadSize.getSplits => bytesRemaining="+bytesRemaining+", thisChunk="+thisChunk );
                }
