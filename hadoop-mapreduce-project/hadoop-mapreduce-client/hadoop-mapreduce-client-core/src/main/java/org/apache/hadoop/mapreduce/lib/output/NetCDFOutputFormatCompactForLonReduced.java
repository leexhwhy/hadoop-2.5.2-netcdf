 ...
 */
public class NetCDFOutputFormatCompactForLonReduced<Text, List> extends FileOutputFormat<Text, List> {
    ...
    public NetCDFOutputFormatCompactForLonReduced(){
        super();
    }
        ...
        public synchronized void write(Text key, List value)
            ...
            try {
                ...
                for( int i = 0; i < lonIndexesSize; i++ ){
                    ...
                    dataLon.setDouble(i, Double.valueOf(Integer.valueOf(currentCumulativeLon)*numChunksPerKey+i));
                }
                ...
                for (int i = 0; i < shape[0]; i++) {
                    for (int j = 0; j < shape[1]; j++) {
                        idx.set(i, j);
                        if( j == 0 ){
                            dataTimeBnds.setDouble(idx, (double)i - 0.5);
                        }else if( j == 1 ){
                            dataTimeBnds.setDouble(idx, (double)i + 0.5);
                        }
                ...
                shape = dataLat.getShape();
                for (int i = 0; i < shape[0]; i++) {
                    dataLat.setDouble(i, (double)i);
                }
                ...
                for (int i = 0; i < shape[0]; i++) {
                    for (int j = 0; j < shape[1]; j++) {
                        idx.set(i, j);
                        if( j == 0 ){
                            dataLatBnds.setDouble(idx, (double)i - 0.5 );
                        }else if( j == 1 ){
                            dataLatBnds.setDouble(idx, (double)i + 0.5 );
                        }
                    }
                ...
                _fs.moveFromLocalFile(new Path(outputFileName), new Path(_output_path + "/rsutlon" + currentCumulativeLon));
            } catch (Exception e) {
