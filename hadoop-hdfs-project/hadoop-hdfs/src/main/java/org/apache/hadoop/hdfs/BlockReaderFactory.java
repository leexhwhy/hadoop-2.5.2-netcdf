...
public class BlockReaderFactory implements ShortCircuitReplicaCreator {
  ...
  public BlockReader build() throws IOException {
    ...
    //LOG.info( "[SAMAN] BlockReaderFactory.build()" );
  ...
  private BlockReader getRemoteBlockReaderFromTcp() throws IOException {
    ...
    //LOG.info("[SAMAN] BlockReaderFactory.getRemoteBlockReaderFromTCp");
  ...
  private BlockReader getRemoteBlockReader(Peer peer) throws IOException {
    //LOG.info("[SAMAN] BlockReaderFactory.getRemoteBlockReader, peer:"+peer.getLocalAddressString()+"|"+peer.getRemoteAddressString());

    if (conf.useLegacyBlockReader) {
      //LOG.info( "[SAMAN] BlockReaderFactory.getRemoteBlockReader, conf.useLegacyBlockReader=true" );
      return RemoteBlockReader.newBlockReader(fileName,
          block, token, startOffset, length, conf.ioBufferSize,
          verifyChecksum, clientName, peer, datanode,
          clientContext.getPeerCache(), cachingStrategy);
    } else {
      //LOG.info( "[SAMAN] BlockReaderFactory.getRemoteBlockReader, conf.useLegacyBlockReader=false" );
