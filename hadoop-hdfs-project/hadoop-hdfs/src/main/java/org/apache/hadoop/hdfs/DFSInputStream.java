...
implements ByteBufferReadable, CanSetDropBehind, CanSetReadahead,
  ...
  private synchronized LocatedBlock   getBlockAt(long offset,
    ...
    if (offset < 0 || offset >= getFileLength()) {
      ...
      //DFSClient.LOG.info( "[SAMAN] DFSInputStream.getBlockAt( long, boolean ) => " +
      //        "offset >= locatedBlocks.getfileLength()!" );
      //System.out.println( "[SAMAN] DFSInputStream.getBlockAt( long, boolean ) => " +
      //        "offset >= locatedBlocks.getfileLength()!" );
  ...
  private int readWithStrategy(ReaderStrategy strategy, int off, int len) throws IOException {
    ...
    if (pos < getFileLength()) {
      //DFSClient.LOG.info("[SAMAN] DFSInputStream.readWithStrategy => pos < getFileLength()");
      //System.out.println("[SAMAN] DFSInputStream.readWithStrategy => pos < getFileLength()");
      int retries = 2;
      while (retries > 0) {
        //DFSClient.LOG.info( "[SAMAN] DFSInputStream.readWithStrategy => inside while" );
        //System.out.println( "[SAMAN] DFSInputStream.readWithStrategy => inside while" );
        try {
          // currentNode can be left as null if previous read had a checksum
          // error on the same block. See HDFS-3067
          if (pos > blockEnd || currentNode == null) {
            //DFSClient.LOG.info( "[SAMAN] DFSInputStream.readWithStrategy( ReaderStrategy, int, int ) " +
  ...
  public synchronized int read(final byte buf[], int off, int len) throws IOException {
    ...
    //DFSClient.LOG.info( "[SAMAN] DFSInputStream.read( byte[], int, int )" );
  ...
  private synchronized boolean seekToBlockSource(long targetPos)
                                                 throws IOException {
    DFSClient.LOG.info( "[SAMAN] DFSInputStream.seekToBlockSource(long)" );
  ...
  public synchronized boolean seekToNewSource(long targetPos) throws IOException {
    DFSClient.LOG.info( "[SAMAN] DFSInputStream.seekToNewSource(long)" );
