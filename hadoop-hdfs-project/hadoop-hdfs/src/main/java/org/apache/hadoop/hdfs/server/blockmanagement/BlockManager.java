...
public class BlockManager {
  ...
  private List<LocatedBlock> createLocatedBlockList(BlockInfo[] blocks,
      final long offset, final long length, final int nrBlocksToReturn,
      final AccessMode mode) throws IOException {

    // SAMAN, Just to test if removing modified blocks really works or not

    int usefulBlocksChecked[] = new int[blocks.length];
    int usefulBlocksSize = blocks.length;
    /*
    if( blocks.length != 1 ) {

      for (int i = 0; i < blocks.length; i++) {
        if (blocks[i].getIsModifiedBlock() == false) {
          usefulBlocksSize++;
        }
      }
      for (int i = 0; i < blocks.length; i++) {
        if (blocks[i].getIsModifiedBlock() == false) {
          usefulBlocksChecked[i] = 1;
        }else{
          usefulBlocksChecked[i] = 0;
        }
      }
    }
    */
    ...
    List<LocatedBlock> results = new ArrayList<LocatedBlock>(usefulBlocksSize);

    do {
      //if( usefulBlocksChecked[curBlk] == 1 )
  ...
  public DatanodeStorageInfo[] chooseTarget(final String src,
    ...
    System.out.println( "[SAMAN][BlockManager][chooseTarget] targets are: " + targetsString  );
  ...
  public BlockInfo addBlockCollection(BlockInfo block, BlockCollection bc, boolean isBlockModified){
    return blocksMap.addBlockCollection(block, bc, isBlockModified);
  }
